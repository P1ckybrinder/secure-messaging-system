...
import { getProductsServer, getFlashSalesServer, getTrendingServer, getNewArrivalsServer, getLuxuryDealsServer, getTopPicksServer, getDailyFindsServer } from "@/lib/server-api"

// ✅ Performance notes:
// - This page is now a Server Component (no "use client").
// - Removed eager prefetch burst on mount (was hurting LCP).
// - Enabled SSR for ProductGrid and key sections so meaningful HTML is sent immediately.
// - Kept heavy/less-critical sections as dynamic if needed, but without ssr:false unless required.
...
const SectionSkeleton = ({ icon: Icon, title, color = "#8B1538" }: { icon?: any; title?: string; color?: string }) => (
  <div className="w-full">
    <div style={{ backgroundColor: color }} className="text-white flex items-center justify-between px-2 sm:px-4 py-1.5 sm:py-2">
    ...
    <div className="p-2">
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2 sm:gap-3">
        {[...Array(4)].map((_, i) => (
          <div key={i} className="bg-white p-2 sm:p-3 rounded-lg">
...
const ProductGridSkeleton = () => (
  <div className="w-full">
    <div className="grid grid-cols-2 gap-2 sm:grid-cols-3 sm:gap-3 md:grid-cols-4 lg:grid-cols-6 lg:gap-4">
      {[...Array(8)].map((_, i) => (
        <div key={i} className="bg-white p-2 sm:p-3">
...
// NOTE: previously ssr:false – allowing SSR reduces LCP.
const ProductGrid = dynamic(() => import("@/components/products/product-grid").then((mod) => mod.ProductGrid), {
  loading: () => <ProductGridSkeleton />,
})

// Keep these dynamically loaded but allow SSR unless there is a hard browser-only dependency.
        ...
        <div className="mx-auto w-full max-w-[1200px] px-2 sm:px-3 md:px-4">
          <div className="grid gap-3 sm:gap-4 md:gap-8 py-2 sm:py-4">
            <section className="rounded-lg bg-white shadow-sm overflow-hidden">
              <FlashSales initialProducts={initialFlashSales} />
            </section>

            <section className="rounded-lg bg-white shadow-sm overflow-hidden">
              <LuxuryDeals initialProducts={initialLuxuryDeals} />
            </section>

            <section className="rounded-lg bg-white shadow-sm overflow-hidden">
              <TopPicks initialProducts={initialTopPicks} />
            </section>

            <section className="rounded-lg bg-white shadow-sm overflow-hidden">
              <NewArrivals initialProducts={initialNewArrivals} />
            </section>

            <section className="rounded-lg bg-white shadow-sm overflow-hidden">
              <TrendingNow initialProducts={initialTrending} />
            </section>

            <section className="rounded-lg bg-white shadow-sm overflow-hidden">
              <DailyFinds initialProducts={initialDailyFinds} />
            </section>
            ...
            <section className="rounded-lg bg-white shadow-sm overflow-hidden">
              <div className="bg-[#8B1538] text-white flex items-center justify-between px-2 sm:px-4 py-1.5 sm:py-2">
                ...
                >
                  View All <span className="inline-block arrow-animate">→</span>
                </Link>
              </div>
              <div className="p-1 sm:p-2">
                <ProductGrid limit={12} initialProducts={initialProducts} />
              </div>
            </section>
